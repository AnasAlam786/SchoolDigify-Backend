...
@get_result_api_bp.route('/api/get_result', methods=["POST"])
@login_required
...
def get_result_api():
    ...
    extra_fields = {
        "StudentsDB": ["STUDENTS_NAME", "FATHERS_NAME", "FATHERS_NAME", "IMAGE",
                       "MOTHERS_NAME", "ADDRESS", "PHONE", 'GENDER', "PEN", "IMAGE"],
...
def _prepare_certificate_students(student_items, session_id):
    student_ids = []

    for item in student_items:
        try:
            student_ids.append(int(item.get('student_id')))
        except (TypeError, ValueError):
            continue

    students = {}
    for student in StudentsDB.query.filter(StudentsDB.id.in_(student_ids)).all():
        students[student.id] = student

    
    student_sessions = {}

    for ss in StudentSessions.query.filter_by( session_id=session_id ).filter(
        StudentSessions.student_id.in_(student_ids) ).all():
        student_sessions[ss.student_id] = ss


    class_ids = []
    for ss in student_sessions.values():
        if ss.class_id:
            class_ids.append(ss.class_id)

    classes = {}
    for c in ClassData.query.filter(ClassData.id.in_(class_ids)).all():
    ...
    for item in student_items:
        ...
        student_session = student_sessions.get(student_id)

        class_name = (
            classes.get(student_session.class_id)
            if student_session and student_session.class_id
            else student.CLASS
        )

        img_src = (
            f'https://lh3.googleusercontent.com/d/{student.IMAGE}=s220'
            if student.IMAGE
            else url_for(
                'static',
                filename='images/default_student.png'
            )
        )
...
@get_result_api_bp.route('/api/get_certificate_html', methods=["POST"])
@login_required
@permission_required('get_result')
def print_certificate_api():
    ...
    if not students or not isinstance(students, list):
        return jsonify({ "error": "students list is required and must be an array." }), 400
    ...
    try:
        ...
        html = render_template('pdf-components/certificates/certificate.html',
                students=students, session_year=session_year,
                school_logo=school_logo,  school_name=school_name)

        return jsonify({'html': str(html)})
    except Exception as e:
        print(f"Error generating certificate: {e}")
        return jsonify({'error': 'An error occurred while generating the certificate.'}), 500
