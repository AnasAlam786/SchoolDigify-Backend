...
@promote_student_api_bp.route('/api/promote_student', methods=["POST"])
@login_required
...
"state": "PROMOTED"
    ...
    try:
        ...
        return jsonify({"error": "Student, promoted roll, or promoted class is not valid!"}), 400
    ...
    if promoted_date:
        try:
            ...
            return jsonify({"error": "Invalid promotion date format. Use 'year-month-day'."}), 400
    else:
    ...
    if promoted_roll not in final_rolls:
        ...
        return jsonify({
            "error": f"Cannot assign this roll no. You can only assign one of these rolls: {allowed_rolls}."
        }), 400
    ...
    if recovered_status == "tc":
        return jsonify({"error": "TC already issued. Promotion not allowed."}), 400

    if recovered_status == "promoted":
        return jsonify({"error": "Already promoted cannot promote again."}), 400

    if recovered_status not in ("active", None, ""):
        return jsonify({"error": "Promotion not allowed."}), 400
    
    # --- Prevent duplicate ---
    if already_promoted and already_promoted.status != "left":
        return jsonify({
            "error": "Student already has an active entry in this session."
        }), 400
    ...
    if not selected_class:
        return jsonify({"error": "Selected class does not exist or is not available."}), 400
    ...
    if (selected_class.grade_level is not None and 
        ...
        return jsonify({"error": "Cannot promote to a lower class."}), 400
    ...
    if roll_conflict:
        return jsonify({"error": "This roll number is already in use in the target class and session."}), 400
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({
            "error": "Failed to promote student due to a roll/class conflict."
        }), 400
        ...
        return jsonify({
            "messaerrorge": "Failed to promote student due to a database error."
        }), 500

    return jsonify({
        "message": "Student promoted successfully",
        "state": "PROMOTED",
        "promoted_student_id": new_session.id,
        "new_roll": new_session.ROLL,
        "new_class_id": new_session.class_id,
        "new_class": selected_class.CLASS,  # ✅ added
