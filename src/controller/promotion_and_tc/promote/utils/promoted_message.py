...
generate_promoted_message_api_bp = Blueprint('generate_promoted_message_api_bp', __name__)


@generate_promoted_message_api_bp.route('/api/promoted-message', methods=["POST"])
@login_required
@permission_required('promote_student')
def generate_promoted_message():
    data = request.json
